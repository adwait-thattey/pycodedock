...
This module works well with multiple parallel threads(in fact it was designed for that purpose) if you **ensure that all the instances get a unique id** (see usage below).  
...
> Now you can use the following commands to use the modules
...
```
#### Parameters:
`unique_code`: `string` A string identifying the container. If you are running multiple simultanious containers, **make sure all of them have a different unique_code**  

`code_file` : `string` The file containg script. (variable 'script' in this case)  

`input_file`: `string` The file containing the input that will be redirected to STDIN when executing the script  
...
#### Step 4 :
Note that at this time, container is not running. The following command that will start the container and execute the code
` >>> status = D.run()
...
Once everything is done, it is important to destroy the container and delete recidue files
`>>> D.destroy()`
...
The module is designed to handle SIGTERM (Cntrl+C) (kill). So if you want to stop the execution of `D.run()` mid-way, you can press Cntrl+C or use the kill command. The module will stop the container , delete all recidue files and then exit.  

**However SIGKILL (kill -9) , EOF/SIGSTOP (Cntrl+D) can not be handled. If you press (Cntrl+D or give a kill -9) the module will terminate without getting a chance to clean. If the container is running, it WILL NOT BE STOPPED and recidue files WILL NOT BE DELETED** 

To repair this you need to manually stop the containers using `docker stop` and delete the recidue folder/files from the directory (_home by default but can be changed in custom settings_). The recidue files are in a folder with the `unique_name` that you provided while creating instance.

**_Failing to do the above step may cause an error when you try to run a script next time_**
...
5. **Lastly, this module is still under alpha stages of development. If you find any bug or vulnerability, please take a few minutes to report it on issues Page**
